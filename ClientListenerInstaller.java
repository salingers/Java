...
public class ClientListenerInstaller
{
	...
	public void install(Class<?> clz) throws Exception
	{
		...
		// 建立代理物件
		Object listenerProxy = Proxy.newProxyInstance(ClientListener.class.getClassLoader(), new Class[]
		{ ClientListener.class },
				(proxy, method, args) -> methods.get(method.getName()).invoke(clz.newInstance(), args));

		// 用代理物件作註冊
		Method addclientListener = queue.getClass().getMethod("addClientListener", ClientListener.class);
	...
	private void collectAnnotatedMethods(Class<?> clz) throws SecurityException
	{
		...
		{
			ClientAdded clientAdded = method.getAnnotation(ClientAdded.class);
			if (clientAdded != null)
			{
				methods.put("clientAdded", method);
			}
			ClientRemoved clientRemoved = method.getAnnotation(ClientRemoved.class);
