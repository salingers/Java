...
public class SelfReductionInterchanged implements ScaleTester {
	private float lookupValues[][];
	private int nRows, nCols, nThreads;
	float sumValue;
	Object lock = new Object();

	private class SelfReductionInterchangedHandler extends SelfLoopHandler {
		SelfReductionInterchangedHandler(int nc, int nt) {
			super(0, nc, 10, nt);
		}
	...
	public float[][] doCalc() {
		SelfReductionInterchangedHandler loop = new SelfReductionInterchangedHandler(nCols, nThreads);
		loop.loopProcess();
		return lookupValues;
	}
}
