...
public class ReductionInterchanged implements ScaleTester {
	private float lookupValues[][];
	private int nRows, nCols, nThreads;
	float sumValue;
	Object lock = new Object();

	private class ReductionInterchangedHandler extends LoopHandler {
		ReductionInterchangedHandler(int nc, int nt) {
			super(0, nc, nt);
		}
	...
	public float[][] doCalc() {
		ReductionInterchangedHandler loop = new ReductionInterchangedHandler(nCols, nThreads);
		loop.loopProcess();
		return lookupValues;
	}
}
