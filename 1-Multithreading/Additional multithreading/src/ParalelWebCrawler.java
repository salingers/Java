...
public class ParalelWebCrawler
{
	...
	public Set<URI> crawl(URI currentLocation, final String needle, AtomicInteger parentInt)
			throws URISyntaxException, MalformedURLException, InterruptedException, ExecutionException
	{
		...
		}
		{
				...
				{ // visitedUrls.add(currentLocation);
					...
					ListenableFuture<Set<URI>> a = service.submit(new Callable<Set<URI>>()
					{
						...
						public Set<URI> call() throws Exception
						{
							...
							/*
							 * i.decrementAndGet(); // parent notify
			...
			/*
			 * while (!i.compareAndSet(0, 0)) { synchronized (i) { i.wait(); } }
			 * 
			 * for (ListenableFuture<Set<URI>> resultListenableFuture : futures)
			 * { Set<URI> result = resultListenableFuture.get(); if
			 * (!result.isEmpty()) { finalResult.addAll(result); } }
			 */
		}
		return finalResult;
	}
	...
	{
		List<? extends Object> filteredLinks = allLinks.stream().map(link ->
		{
			try
			{
	...
	private static String downloadContents(URI startLocation) throws URISyntaxException
	{
		...
		try
		{
			...
			String contents = EntityUtils.toString(response.getEntity(), StandardCharsets.UTF_8);
			return contents;
		}
		catch (IOException e)
		{
